...
program
  ...
  .option(
    "-t, --testsJsonPath <testsJsonPath>",
    "Path to JSON file containing all ACT Rules testcases"
  )
  .option(
    "-d, --dir <dir>",
    "Output directory of generated implementation report"
  )
  .parse(process.argv);
...
async function init({
   ...
   * Get `implementation`
   */
  const actRulesPkgJson = JSON.parse(
    fs.readFileSync(actRulesPkg, { encoding: "utf-8" })
  );
  const { testcases } = JSON.parse(
    fs.readFileSync(testsJsonPath, { encoding: "utf-8" })
  );
  const data = await getImplementationForReport(
    framedReport,
    testcases,
    actRulesPkgJson
  );
