...
export function getImplementationSet(assertions: EarlAssertion[], testcases: Testcase[]) {
  ...
  const assertsByImplementationId = assertions.reduce((assertsByRule: AssertionGroup, assertion) => {
    ...
    if (!test) {
      debug('getImplementationSet')(`Skipped! Could not find 'test' in assertion ${JSON.stringify(assertion)}`)
  ...
  const allImplementations: Implementation[] = Object.entries(assertsByImplementationId)
...
function findConsistency(
...
): { consistency: Consistency; implementations: Implementation[] } {
  ...
  if (partialImpl.length > 0) {
    const consistency = isConsistentImplementationSet(partialImpl) ? 'consistent' : 'partially-consistent'
    return {
      consistency,
