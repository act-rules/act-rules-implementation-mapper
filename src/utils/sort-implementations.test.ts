...
describe('sortImplementations', () => {
  it('prioritises no meaningful results after number of inconsistencies', () => {
    const { implementationMaps } = testDataFromTables({
      expected: ['passed', 'passed', 'failed', 'failed', 'failed', 'inapplicable'],
      // Nothing tested, should come last
      impl0: ['untested', 'cantTell', 'untested', 'cantTell', 'untested', 'cantTell'],
  ...
  it('sorts least number of partially consistent after consistent with untested', () => {
    const { implementationMaps } = testDataFromTables({
      ...
      impl0: ['passed', 'passed', 'failed', 'cantTell', 'passed', 'inapplicable'],
      // 2 missing on failed  - 1
      impl1: ['passed', 'passed', 'failed', 'passed', 'inapplicable', 'inapplicable'],
      // 1 untested un failed  - 3
      impl2: ['passed', 'passed', 'failed', 'untested', 'failed', 'inapplicable'],
    })
  ...
  it('sorts least number of untested after cantTells', () => {
    const { implementationMaps } = testDataFromTables({
      expected: ['passed', 'passed', 'failed', 'failed', 'failed', 'inapplicable'],
      // 1 untested
      impl0: ['untested', 'passed', 'failed', 'failed', 'failed', 'inapplicable'],
      // 2 untested
      impl1: ['untested', 'passed', 'untested', 'failed', 'failed', 'inapplicable'],
      // no untested, 3 cantTells
      impl2: ['cantTell', 'passed', 'cantTell', 'cantTell', 'failed', 'inapplicable'],
    })
  ...
  it('sorts least number of cantTells', () => {
    const { implementationMaps } = testDataFromTables({
      expected: ['passed', 'passed', 'failed', 'failed', 'failed', 'inapplicable'],
      // 1 cantTell
      impl0: ['cantTell', 'passed', 'failed', 'failed', 'failed', 'inapplicable'],
      // 2 cantTell
      impl1: ['passed', 'cantTell', 'cantTell', 'failed', 'failed', 'inapplicable'],
      // 0 cantTells
