...
describe('findAssertions', () => {
  ...
  it('throws when passed an invalid argument', () => {
    const err = (data: any) => new TypeError(`JSON report must be an object or array, got '${data}'`)
    // @ts-ignore
  ...
  it('returns an array of assertions from an array of objects', async () => {
    const id1 = 'http://myAssertions.org/foo'
    const id2 = 'http://myAssertions.org/bar'
    ...
    assertions.forEach(assertion => expect(assertion['@type']).toBe('earl:Assertion'))
  ...
  it('returns an array of assertions from an object', async () => {
    ...
    const assertions = await findAssertions({
      ...
      '@reverse': {
        [`${earl}subject`]: [{ '@type': `${earl}Assertion`, '@id': id1 }, { '@type': `${earl}Assertion`, '@id': id2 }],
      },
    })
    expect(assertions.length).toBe(2)
    assertions.forEach(assertion => expect(assertion['@type']).toBe('earl:Assertion'))
